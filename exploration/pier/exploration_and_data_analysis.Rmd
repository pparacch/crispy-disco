...
From the data below we can see that there are `r dim(renta_an_1)[1]` clients where it is posible to recove the missing renta just just using the available renta for the same client not NA. On the other side there are `r dim(renta_an_2)[1]` clients where such information cannot be found.

`r kable(renta_an_1, format = "markdown",caption = "Summary OF CLIENTS WITH Missing renta for some entries")`
`r kable(head(renta_an_2, 20), format = "markdown",caption = "Summary OF CLIENTS (first 20s) WITH Missing renta all entries")`

```{r exampleOfMissingRentaWIthSomeValues}
df_train[df_train$ncodpers == 183439, c(1:30), with = F]
```

```{r exampleOfMissingRentaWithAllMissing}
